...
require_relative 'colorize_cards'
...
@@shoe_rule, @@hard_mode = false, false
  ...
  def check_hand(call_it = false)
    you_lost if @Bust.(:player)
    you_lost if @Blackjack.(:dealer) && @@hard_mode == true
    you_won  if @Blackjack.(:player)
    you_won  if @Bust.(:dealer)
    you_won  if @SixCardWin.(:player)
    you_lost if @Blackjack.(:dealer)
